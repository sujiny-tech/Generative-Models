...
d_loss=tf.reduce_mean(-(tf.log(y_data)+tf.log(1-y_fake))) ## tf.reduce_mean
g_loss=tf.reduce_mean(-tf.log(y_fake)) ##
...
for epoch in range(n_epoch):
    ...
    if epoch==0 or (epoch+1)%1000==0:
        ...
        plt.savefig('./GAN_10000/{}.png'.format(str(epoch).zfill(4)),bbox_inches='tight')
        #plt.close()
