...
def plot_faces(faces, n):
    for i in range(n * n):
        ...
        plt.axis('off')
        plt.imshow(faces[i])
    plt.savefig('./origin2_ver2_no_scale_84size/plot_generated_{}.png'.format(str(n)),bbox_inches='tight')
    plt.close()
...
def save_plot(examples, epoch, n=5):
    ...
    plt.savefig('./origin2_ver2_no_scale_84size/{}.png'.format(str(epoch).zfill(4)),bbox_inches='tight')
    plt.close()
...
def plot_generated(examples, n):
    ...
    for i in range(n * n):
        ...
        # plot raw pixel data
        plt.imshow(examples[i, :, :])
    plt.savefig('./origin2_ver2_no_scale_84size/plot_generated_{}.png'.format(str(examples)),bbox_inches='tight')
...
###load train model
new_saver=tf.train.import_meta_graph('./origin2_ver2_no_scale_84size/generator_model_110.ckpt.meta')
new_saver.restore(sess, tf.train.latest_checkpoint('./origin2_ver2_no_scale_84size/'))

total_batch = int(sample_size / batch_size) #50000/128=390
half_batch=int(batch_size/2) #195

for epoch in range(110,total_epoch):#100
    ...
    if epoch==0 or (epoch+1)%5==0:
        ...
        saver.save(sess, './origin2_ver2_no_scale_84size/generator_model_%03d.ckpt' % (epoch+1))
